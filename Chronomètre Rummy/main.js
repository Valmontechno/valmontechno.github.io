...
playersButton.addEventListener('click', changePlayersNumber);
...
playersButton.setAttribute('value', playersNumber);

const alarmSound = new Audio('sounds/power-down.mp3');
...
function start(e) {
  ...
  if(currentPlayer > playersNumber)
    currentPlayer = 1;
  spinnerProgressBar.style.stroke = `var(--player-${currentPlayer}-color)`;
...
function changePlayersNumber() {
  playersNumber++;
  if(playersNumber > 4)
    playersNumber = 2
  playersButton.setAttribute('value', playersNumber);
  currentPlayer = 0;
  spinnerProgressBar.style.stroke = 'var(--player-1-color)';
  if(!paused)
    pause();
}
